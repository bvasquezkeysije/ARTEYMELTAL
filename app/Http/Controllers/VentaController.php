...
class VentaController extends Controller
{
    ...
    public function index()
    {
        ...
        $ventas = Venta::query()
            ...
            ->when($miCaja && $caja, function ($query) use ($caja) {
                $query->where('caja_apertura_id', $caja->id);
            })
            ->orderByDesc('id')
            ->paginate(12)
            ->withQueryString();
        ...
        return view('ventas.index', compact('ventas', 'busqueda', 'tipo', 'caja', 'cajasAbiertas', 'miCaja') + ['sinCaja' => false]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $datosBase = $request->validate([
            ...
            'forma_pago' => ['required', 'string', 'in:efectivo,yape,plin,transferencia,tarjeta,mixto'],
    ...
    public function comprobante(Venta $venta)
    {
        ...
        $baseImponible = round($totalComprobante / 1.18, 2);
        $igv = round($totalComprobante - $baseImponible, 2);
    ...
    private function registrarVentaStock(Request $request, array $datosBase, int $cajaAperturaId)
    {
        ...
        $formaPago = $datosBase['forma_pago'];
        $montoRecibido = (float) ($datosBase['monto_recibido'] ?? $total);
        $esEfectivo = in_array($formaPago, ['efectivo', 'mixto']);
        $esDigital = in_array($formaPago, ['yape', 'plin', 'transferencia', 'tarjeta']);
        ...
        if ($erroresComprobante !== []) {
            ...
            DB::transaction(function () use ($lineas, $datos, $datosBase, $total, $request, $cajaAperturaId, $formaPago, $montoRecibido, $esEfectivo, $esDigital) {
            $vuelto = ($esEfectivo && $montoRecibido > $total) ? round($montoRecibido - $total, 2) : 0;
            $montoEfectivo = $esEfectivo ? round($montoRecibido - $vuelto, 2) : 0;
            $montoDigital = $esDigital ? $total : 0;

            if ($formaPago === 'mixto') {
                $montoEfectivo = round($montoRecibido, 2);
                $montoDigital = round($total - $montoEfectivo, 2);
                if ($montoDigital < 0) $montoDigital = 0;
            }
