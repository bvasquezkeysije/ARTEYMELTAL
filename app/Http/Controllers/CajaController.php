...
class CajaController extends Controller
{
    ...
    public function cerrar(Request $request, CajaApertura $cajaApertura)
    {
        ...
        $totalEfectivoBruto = $cajaApertura->ventas()->sum('monto_efectivo');
        $totalVuelto = $cajaApertura->ventas()->sum('vuelto');
        $totalDigital = $cajaApertura->ventas()->sum('monto_digital');

        $cajaApertura->update([
            'fecha_cierre' => now(),
            'monto_final' => $request->monto_final,
            'total_ventas' => $totalVentas,
            'total_efectivo' => $totalEfectivoBruto - $totalVuelto,
