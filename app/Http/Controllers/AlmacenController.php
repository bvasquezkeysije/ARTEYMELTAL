...
class AlmacenController extends Controller
{
    ...
    public function recibirPedido(Request $request, Pedido $pedido)
    {
        ...
        $vendedores = User::whereHas('rol', fn($q) => $q->whereIn('nombre', ['administrador', 'vendedor']))
            ->where('activo', true)
            ->get();
        foreach ($vendedores as $vendedor) {
            NotificationController::create(
                userId: $vendedor->id,
                type: 'pedido_en_almacen',
                title: 'Pedido recibido en almacen',
                body: "El pedido {$pedido->codigo} - {$pedido->nombre_cliente} ha sido recibido en almacen.",
                icon: '<svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0v10l-8 4m8-14l-8 4m-8-4v10l8 4m0-10v10"/></svg>',
    ...
    public function entregarCliente(Request $request, Pedido $pedido)
    {
        ...
        foreach ($vendedores as $vendedor) {
            NotificationController::create(
                ...
                body: "El pedido {$pedido->codigo} - {$pedido->nombre_cliente} ha sido entregado al cliente.",
