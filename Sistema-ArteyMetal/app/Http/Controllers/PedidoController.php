...
class PedidoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($datos, $request, $cajaAperturaId) {
            ...
            $venta = Venta::create([
                ...
                'monto_efectivo' => $datos['metodo_pago'] === 'efectivo' ? $datos['monto_adelanto'] : 0,
                'monto_digital' => in_array($datos['metodo_pago'], ['yape','plin','tarjeta','transferencia']) ? $datos['monto_adelanto'] : 0,
    ...
    public function confirmarPagoFinal(Request $request, Pedido $pedido)
    {
        ...
        DB::transaction(function () use ($pedido, $request) {
            ...
            $venta = Venta::create([
                ...
                'monto_efectivo' => $request->input('metodo_pago') === 'efectivo' ? $saldoPendiente : 0,
                'monto_digital' => in_array($request->input('metodo_pago'), ['yape','plin','tarjeta','transferencia']) ? $saldoPendiente : 0,
    ...
    private function validarPedido(Request $request, ?int $pedidoId = null): array
    {
        return $request->validate([
            ...
            'tipo_entrega' => ['required', 'string', 'in:local'],
            'direccion_entrega' => ['nullable', 'string', 'max:255'],
            'referencia_entrega' => ['nullable', 'string', 'max:255'],
            'distrito_entrega' => ['nullable', 'string', 'max:120'],
            'codigo_postal_entrega' => ['nullable', 'string', 'max:12'],
            'nombre_recibe' => ['nullable', 'string', 'max:120'],
            'telefono_recibe' => ['nullable', 'string', 'max:20'],
