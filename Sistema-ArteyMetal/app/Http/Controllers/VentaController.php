...
class VentaController extends Controller
{
    ...
    private function registrarVentaStock(Request $request, array $datosBase, int $cajaAperturaId)
    {
        ...
        $montoRecibido = (float) $request->input("monto_recibido", $total);

        $montoEfectivo = $formaPago === "efectivo" ? $total : 0.0;
        $montoDigital = $formaPago !== "efectivo" ? $total : 0.0;
        $vuelto = $formaPago === "efectivo" ? max(0.0, $montoRecibido - $total) : 0.0;

        DB::transaction(function () use ($lineas, $datos, $datosBase, $total, $request, $cajaAperturaId, $formaPago, $montoEfectivo, $montoDigital, $vuelto) {
            $venta = Venta::create([
