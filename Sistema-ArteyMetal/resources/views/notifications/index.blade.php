            ...
            <div class="divide-y divide-[#e5dec8]">
                @forelse ($notifications as $notification)
                    <div class="flex items-start gap-4 px-5 py-3 @if(!$notification->is_read) bg-[#fffbf5] @endif">
                        <span class="mt-0.5 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full @if(!$notification->is_read) bg-[#b9943d]/20 text-[#7a5b25] @else bg-gray-100 text-gray-400 @endif">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-start justify-between gap-2">
                                <p class="text-sm font-semibold @if(!$notification->is_read) text-gray-900 @else text-gray-500 @endif">{{ $notification->title }}</p>
