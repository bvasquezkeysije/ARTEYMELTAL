                        ...
                        @forelse($pedidos as $pedido)
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @endphp
                                    @if($archivosCliente->isNotEmpty())
                                        <div class="mb-1">
                                            <span class="text-xs font-semibold text-amber-700">Cliente:</span>
                                    ...
                                    @if($archivosDisenador->isNotEmpty())
                                        <div>
                                            <span class="text-xs font-semibold text-sky-700">Disenador:</span>
                                            <div class="flex flex-wrap gap-1 mt-0.5">
                                                @foreach($archivosDisenador as $archivo)
                                                    <a href="{{ asset('storage/' . $archivo->archivo_path) }}" target="_blank"
                                                       class="inline-flex items-center gap-1 rounded-md bg-sky-100 px-2 py-1 text-xs text-sky-700 hover:bg-sky-200"
                                                       title="{{ $archivo->nombre_original }}">
                                                        <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                                        {{ Str::limit($archivo->nombre_original, 20) }}
                                                    </a>
                                    ...
                                    @endif
                                    @if($archivosCliente->isEmpty() && $archivosDisenador->isEmpty())
        ...
        <div x-data="{
            ...
            <div @@click.outside="open = false" class="mx-4 flex max-h-[85vh] w-full max-w-3xl flex-col rounded-2xl bg-white shadow-xl">
                ...
                <div class="flex-1 overflow-y-auto px-6 py-4">
                    ...
                    <div class="mt-3">
                        <p class="text-xs font-semibold uppercase tracking-wider text-sky-700">Mis disenos</p>
                        <div class="mt-1">
                            ...
                            @if($archivosDisenador->isNotEmpty())
                                <div class="flex flex-wrap gap-2">
                                    @foreach($archivosDisenador as $archivo)
                                        <a href="{{ asset('storage/' . $archivo->archivo_path) }}" target="_blank"
                                           class="inline-flex items-center gap-1 rounded-lg border border-[#b8d4e8] px-3 py-1.5 text-sm text-sky-800 hover:bg-[#eef7ff]">
                                            {{ $archivo->nombre_original }}
                                            <span class="text-[#b0b0b0]">({{ round($archivo->tamano_bytes / 1024) }} KB)</span>
                                        </a>
                                    @endforeach
                                </div>
                            @else
        ...
        <div x-data="{ open: false }"
            ...
            <div @@click.outside="open = false" class="mx-4 w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
                ...
                <form action="{{ route('diseno.update', $pedido) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    @if($archivosCliente->isNotEmpty())
                        <div class="mb-3">
                            <p class="mb-1 text-xs font-semibold text-amber-700">Archivos del cliente:</p>
                            <ul class="space-y-1">
                                @foreach($archivosCliente as $archivo)
                    ...
                    @if($archivosDisenador->isNotEmpty())
                        <div class="mb-3">
                            <p class="mb-1 text-xs font-semibold text-sky-700">Tus disenos anteriores:</p>
                            <ul class="space-y-1">
                                @foreach($archivosDisenador as $archivo)
                                    <li class="flex items-center gap-2 text-xs text-[#555]">
                                        <svg class="h-3 w-3 shrink-0 text-sky-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                        <a href="{{ asset('storage/' . $archivo->archivo_path) }}" target="_blank" class="text-sky-700 underline hover:text-sky-900">
                                            {{ $archivo->nombre_original }}
                                        </a>
                                        <span class="text-[#bbb]">({{ round($archivo->tamano_bytes / 1024) }} KB)</span>
                                    </li>
