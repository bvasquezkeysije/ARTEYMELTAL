<x-app-layout>
    ...
    <div x-data="{ modalVenta: false, ventaVista: null, modalComprobante: false, urlComprobante: '', filtrosAbiertos: false, selectorCajaAbierto: {{ (!$caja && $cajasAbiertas->isNotEmpty()) ? 'true' : 'false' }}, sinCajaAbierto: {{ (!$caja && $cajasAbiertas->isEmpty()) ? 'true' : 'false' }} }" class="space-y-5">
        <div class="rounded-2xl border border-[#e5dec8] bg-white p-4 shadow-sm">
            ...
            <form x-show="filtrosAbiertos" x-transition method="GET" action="{{ route('ventas.index') }}" class="mt-4 flex flex-wrap items-end gap-4 border-t border-[#efe7d2] pt-4">
                ...
                @if($caja)
                <div>
                    <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-gray-600">Caja</label>
                    <select name="mi_caja" class="rounded-xl border border-[#d1be8a] bg-white px-4 py-2.5 text-sm text-gray-700 shadow-sm">
                        <option value="">Todas</option>
                        <option value="1" @selected(($miCaja ?? '') == '1')>Solo mi caja</option>
                    </select>
                </div>
                @endif
                        ...
                        @forelse ($ventas as $venta)
                            @php
                                $ventaVistaData = [
                                    ...
                                    'metodo_pago' => ucfirst($venta->metodo_pago ?? 'Efectivo'),
                                    'efectivo' => $venta->monto_efectivo ? 'S/ ' . number_format((float) $venta->monto_efectivo, 2) : '-',
                                    'digital' => $venta->monto_digital ? 'S/ ' . number_format((float) $venta->monto_digital, 2) : '-',
                                    'vuelto' => $venta->vuelto ? 'S/ ' . number_format((float) $venta->vuelto, 2) : '-',
                                ...
                                <td class="px-4 py-3">
                                    @php
                                        ...
                                        $badgeClass = match($metodoPago) {
                                            ...
                                            'yape', 'plin' => 'bg-sky-100 text-sky-700',
                                    ...
                                    @endphp
                                    <span class="rounded-full px-2.5 py-0.5 text-xs font-medium {{ $badgeClass }}">{{ ucfirst($metodoPago) }}</span>
                                </td>
        ...
        <template x-teleport="body">
            ...
            <div class="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-[#e5dec8] bg-white shadow-xl" @click.stop>
                ...
                <div class="grid gap-4 p-5 md:grid-cols-2" x-show="ventaVista">
                    ...
                    <div>
                        <p class="text-xs uppercase tracking-[0.2em] text-[#8a6a2e]">Metodo pago</p>
                        <p class="mt-1 text-[#1f1f1f]" x-text="ventaVista?.metodo_pago"></p>
                    </div>
                    <div>
                        <p class="text-xs uppercase tracking-[0.2em] text-emerald-700">Efectivo</p>
                        <p class="mt-1 text-emerald-700" x-text="ventaVista?.efectivo"></p>
                    </div>
                    <div>
                        <p class="text-xs uppercase tracking-[0.2em] text-sky-700">Digital</p>
                        <p class="mt-1 text-sky-700" x-text="ventaVista?.digital"></p>
                    </div>
                    <div x-show="ventaVista?.vuelto && ventaVista.vuelto !== '-'">
                        <p class="text-xs uppercase tracking-[0.2em] text-rose-600">Vuelto</p>
                        <p class="mt-1 text-rose-600" x-text="ventaVista?.vuelto"></p>
                    </div>
