<x-app-layout>
    ...
    <div x-data="{ modalPersonalizacion: @js($errors->any()), confirmDerivar: false }" class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div class="mt-5">
            <p class="text-xs font-semibold uppercase tracking-wider text-amber-700">Archivos del cliente</p>
            @php $archivosCliente = $pedido->archivosDiseno->where('tipo', 'cliente'); @endphp
            @if($archivosCliente->isNotEmpty() || $pedido->archivo_diseno_path)
        ...
        <div class="mt-3">
            <p class="text-xs font-semibold uppercase tracking-wider text-sky-700">Disenos del disenador</p>
            @php $archivosDisenador = $pedido->archivosDiseno->where('tipo', 'disenador'); @endphp
            @if($archivosDisenador->isNotEmpty())
                <div class="mt-1 space-y-2">
                    @foreach($archivosDisenador as $archivo)
                        <a class="inline-flex rounded-lg border border-sky-200 px-3 py-1.5 text-sm font-medium text-sky-700 hover:bg-sky-50" href="{{ asset('storage/' . $archivo->archivo_path) }}" target="_blank">
                            {{ $archivo->nombre_original }}
                        </a>
                    @endforeach
                </div>
        ...
        <div class="mt-6 flex flex-wrap gap-2">
            ...
            @if(auth()->user()->tienePermiso('pedidos.gestionar'))
                ...
                @endif
                @if(in_array($rol, ['administrador', 'vendedor'], true) && $pedido->estado === 'registrado')
                    <button type="button" @click="confirmDerivar = true" class="rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-amber-600">Derivar a produccion</button>
                @endif
            @endif
        ...
        {{-- Modal confirmar derivar a produccion --}}
        <div x-show="confirmDerivar" x-cloak x-transition class="fixed inset-0 z-[90] flex items-center justify-center bg-black/45 p-4" @click.self="confirmDerivar = false">
            <div class="w-full max-w-sm overflow-hidden rounded-2xl border border-amber-200 bg-white shadow-2xl">
                <div class="flex items-center justify-between border-b border-amber-100 bg-amber-50 px-5 py-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-base font-semibold text-amber-800">Derivar a produccion</h3>
                    </div>
                    <button type="button" @click="confirmDerivar = false" class="rounded-lg px-2 py-1 text-sm text-amber-500 hover:bg-amber-100">Cerrar</button>
                </div>
                <div class="px-5 py-5">
                    <p class="text-sm text-gray-700">¿Estas seguro de derivar el pedido <strong>{{ $pedido->codigo }}</strong> a produccion?</p>
                </div>
                <div class="flex justify-end gap-2 border-t border-amber-100 px-5 py-3">
                    <button type="button" @click="confirmDerivar = false" class="rounded-xl border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100">Cancelar</button>
                    <form method="POST" action="{{ route('pedidos.derivar', $pedido) }}">
                        @csrf
                        <input type="hidden" name="destino" value="produccion">
                        <button type="submit" class="rounded-xl bg-amber-500 px-4 py-2 text-sm font-medium text-white hover:bg-amber-600">Confirmar</button>
                    </form>
                </div>
            </div>
        </div>
