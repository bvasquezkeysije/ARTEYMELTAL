                    ...
                    <tbody>
                        @forelse ($aperturas as $apertura)
                            <tr class="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-emerald-700">{{ $apertura->ventas_sum_monto_efectivo ? 'S/ '.number_format(max(0, $apertura->ventas_sum_monto_efectivo - ($apertura->ventas_sum_vuelto ?? 0)), 2) : '-' }}</td>
                                <td class="px-4 py-3 text-rose-600">{{ $apertura->ventas_sum_vuelto ? 'S/ '.number_format($apertura->ventas_sum_vuelto, 2) : '-' }}</td>
                                <td class="px-4 py-3 text-sky-700">{{ $apertura->ventas_sum_monto_digital ? 'S/ '.number_format($apertura->ventas_sum_monto_digital, 2) : '-' }}</td>
                                @php
                                    $netoEfectivo = max(0, ($apertura->ventas_sum_monto_efectivo ?? 0) - ($apertura->ventas_sum_vuelto ?? 0));
                                    $totalFinal = $apertura->estado === 'cerrada'
                                        ? ($apertura->monto_final ?? 0) + ($apertura->ventas_sum_monto_digital ?? 0)
                                        : $apertura->monto_inicial + $netoEfectivo + ($apertura->ventas_sum_monto_digital ?? 0);
                                @endphp
                                <td class="px-4 py-3 font-semibold text-gray-900">S/ {{ number_format($totalFinal, 2) }}</td>
                                <td class="px-4 py-3">
                                    ...
                                    <div class="flex justify-end gap-2">
                                        ...
                                        $detalleData = [
                                            ...
                                            'usuario' => $apertura->usuario?->name ?? '-',
                                                ...
                                                'ventas' => $apertura->ventas_count ?? 0,
                                                'neto_efectivo' => $apertura->ventas_sum_monto_efectivo ? 'S/ '.number_format(max(0, $apertura->ventas_sum_monto_efectivo - ($apertura->ventas_sum_vuelto ?? 0)), 2) : '-',
                                                'vuelto' => $apertura->ventas_sum_vuelto ? 'S/ '.number_format($apertura->ventas_sum_vuelto, 2) : '-',
                                                'digital' => $apertura->ventas_sum_monto_digital ? 'S/ '.number_format($apertura->ventas_sum_monto_digital, 2) : '-',
            ...
            <div x-show="cerrarId === {{ $apertura->id }}" x-transition class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display: none;">
                <div class="w-full max-w-md rounded-2xl border border-gray-200 bg-white shadow-xl" @click.stop>
                    ...
                    @php
                        ...
                        $netoEfectivo = $totalEfectivo - $totalVuelto;
                        $efectivoEnCaja = $apertura->monto_inicial + $netoEfectivo;
                    @endphp
                    <form method="POST" action="{{ route('cajas.cerrar', $apertura) }}" class="p-5 space-y-4">
                        ...
                        <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 space-y-2">
                            ...
                            <div class="flex justify-between text-sm text-gray-600">
                                ...
                                <span class="font-medium text-emerald-700">S/ {{ number_format($totalEfectivo, 2) }}</span>
                            </div>
                            <div class="flex justify-between text-sm text-gray-600">
                                <span>- Vuelto</span>
                                <span class="font-medium text-rose-600">S/ {{ number_format($totalVuelto, 2) }}</span>
                            </div>
                            <hr class="border-gray-300">
                            <div class="flex justify-between text-base font-bold text-gray-900">
                                <span>Efectivo en caja</span>
                                <span class="text-emerald-700">S/ {{ number_format($efectivoEnCaja, 2) }}</span>
                            </div>
                            <div class="flex justify-between text-sm text-gray-600">
                                <span>Digital</span>
                                <span class="font-medium text-sky-700">S/ {{ number_format($totalDigital, 2) }}</span>
                            </div>
                            <hr class="border-gray-300">
                            <div class="flex justify-between text-base font-bold text-gray-900">
                                <span>Total general (efectivo + digital)</span>
                                <span>S/ {{ number_format($efectivoEnCaja + $totalDigital, 2) }}</span>
                            </div>
                        </div>
                        <div>
                            <label for="monto_final_{{ $apertura->id }}" class="mb-2 block text-sm font-medium text-gray-700">Monto final en caja <span class="text-emerald-700">(solo efectivo)</span></label>
                            <input id="monto_final_{{ $apertura->id }}" name="monto_final" type="number" step="0.01" min="0" required value="{{ number_format($efectivoEnCaja, 2, '.', '') }}" class="block w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-gray-900" placeholder="Ingresa el efectivo fisico contado" />
                        </div>
                        <div>
        ...
        <div x-show="detalleCaja" x-transition class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display: none;">
            <div class="w-full max-w-lg rounded-2xl border border-gray-200 bg-white shadow-xl" @click.stop>
                ...
                <div class="grid gap-4 p-5 md:grid-cols-2">
                    ...
                    <div>
                        <p class="text-xs uppercase tracking-[0.2em] text-emerald-700">Efectivo (neto)</p>
                        <p class="mt-1 text-emerald-700" x-text="detalleCaja?.neto_efectivo"></p>
                    </div>
                    <div>
                        <p class="text-xs uppercase tracking-[0.2em] text-rose-600">Vuelto</p>
                        <p class="mt-1 text-rose-600" x-text="detalleCaja?.vuelto"></p>
                    </div>
                    <div>
                        <p class="text-xs uppercase tracking-[0.2em] text-sky-700">Digital</p>
                        <p class="mt-1 text-sky-700" x-text="detalleCaja?.digital"></p>
                    </div>
                    <div>
                        <p class="text-xs uppercase tracking-[0.2em] text-gray-500">Monto final (efectivo)</p>
                        <p class="mt-1 text-gray-900" x-text="detalleCaja?.final"></p>
                    </div>
                    <div>
                        <p class="text-xs uppercase tracking-[0.2em] text-gray-900 font-semibold">Total final</p>
                        <p class="mt-1 font-bold text-gray-900" x-text="detalleCaja?.total_final"></p>
                    </div>
