...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full">
    ...
    <body class="m-0 min-h-dvh overflow-hidden bg-[#f4f4f3] antialiased" style="font-family:'Manrope',sans-serif;">
        <div id="app-shell" x-data="{
                ...
                initModals() {
                    @if($errors->any())
                        this.errorMessages = @js($errors->all());
                        this.showErrorModal = true;
                    @endif
                    @if(session()->has('ok'))
                        this.successMessage = @js(session('ok'));
                    ...
                    <header class="shrink-0 border-b border-[#e0e0de] bg-white">
                        <div class="flex items-center justify-between px-4 py-4 sm:px-6">
                            ...
                            <div class="relative" x-data="{
                                ...
                                x-init="init()"
                                @keydown.escape.window="openPerfil = false; openAyuda = false"
                            >
                                <div class="flex items-center gap-2">
                                    <a
                                        href="{{ route('notificaciones.index') }}"
                                        class="relative inline-flex items-center justify-center rounded-xl border border-[#d8cfb8] bg-[#fffdf7] px-3 py-2 text-[#666] hover:bg-[#fff7e7] hover:text-[#3b2e11] transition-colors"
                                    ...
                                    >
                                        ...
                                        <span x-show="notifCount > 0" x-cloak class="ml-1 inline-flex h-2.5 w-2.5 rounded-full bg-red-500"></span>
                                    </a>
            ...
            >
                <div class="w-full max-w-lg overflow-hidden rounded-2xl border border-rose-200 bg-white shadow-2xl">
                    <div class="flex items-center justify-between border-b border-rose-100 bg-rose-50 px-5 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-base font-semibold text-red-800">Errores de validacion</h3>
                        </div>
                        <button type="button" @click="showErrorModal = false" class="rounded-lg px-2 py-1 text-sm text-red-500 hover:bg-red-100">Cerrar</button>
                    </div>
                    ...
                    <div class="border-t border-rose-100 px-5 py-3 text-right">
                        <button type="button" @click="showErrorModal = false" class="rounded-xl bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700">Cerrar</button>
                    </div>
                </div>
            </div>
            ...
            >
                <div class="w-full max-w-md overflow-hidden rounded-2xl border border-emerald-200 bg-white shadow-2xl">
                    <div class="flex items-center justify-between border-b border-emerald-100 bg-emerald-50 px-5 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-base font-semibold text-emerald-800">Operacion exitosa</h3>
                        </div>
                        <button type="button" @click="showSuccessModal = false" class="rounded-lg px-2 py-1 text-sm text-emerald-500 hover:bg-emerald-100">Cerrar</button>
                    </div>
                    <div class="px-5 py-5">
                        <p class="text-sm text-emerald-700" x-text="successMessage"></p>
                    </div>
                    <div class="border-t border-emerald-100 px-5 py-3 text-right">
                        <button type="button" @click="showSuccessModal = false" class="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700">Cerrar</button>
                    </div>
                </div>
            </div>
        </div>
